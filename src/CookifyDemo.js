...
import { FaSearch, FaUser, FaClipboardList, FaSun, FaMoon, FaPalette, FaFont, FaChevronDown, FaChevronUp, FaTimes, FaClock, FaExchangeAlt, FaPlus, FaCalendarAlt, FaChevronRight, FaChevronLeft, FaUtensils, FaShoppingCart, FaCheck, FaFileDownload, FaTrash } from "react-icons/fa";
...
// Категории ингредиентов для группировки в списке покупок
const INGREDIENT_CATEGORIES = {
  vegetables: { ru: "Овощи и зелень", en: "Vegetables & Greens", keywords: ["помидор", "огурец", "лук", "чеснок", "перец", "морковь", "картофель", "капуста", "салат", "зелень", "петрушка", "укроп", "базилик", "шпинат", "руккола", "томат", "баклажан", "кабачок", "тыква", "свекла", "редис"] },
  fruits: { ru: "Фрукты и ягоды", en: "Fruits & Berries", keywords: ["яблок", "груш", "банан", "апельсин", "лимон", "ягод", "клубник", "малин", "черник", "виноград", "киви", "манго", "ананас", "персик", "абрикос", "слив"] },
  meat: { ru: "Мясо и птица", en: "Meat & Poultry", keywords: ["курин", "говяд", "свинин", "индейк", "утк", "бекон", "колбас", "сосиск", "фарш", "ветчин", "мяс"] },
  fish: { ru: "Рыба и морепродукты", en: "Fish & Seafood", keywords: ["рыб", "лосос", "семг", "тунец", "форель", "треск", "креветк", "кальмар", "мидии", "морепродукт", "икр"] },
  dairy: { ru: "Молочные продукты", en: "Dairy", keywords: ["молок", "сливк", "сметан", "йогурт", "кефир", "творог", "сыр", "масл сливочн", "ряженк"] },
  grains: { ru: "Крупы и макароны", en: "Grains & Pasta", keywords: ["рис", "гречк", "овсян", "пшен", "перловк", "макарон", "спагетти", "паста", "лапш", "крупа"] },
  bakery: { ru: "Хлеб и выпечка", en: "Bakery", keywords: ["хлеб", "булк", "батон", "лаваш", "пит", "тортилья", "багет"] },
  spices: { ru: "Специи и приправы", en: "Spices & Seasonings", keywords: ["соль", "перец", "специ", "приправ", "паприк", "кумин", "кориандр", "корица", "ванил", "мускат", "карри", "имбир", "куркум"] },
  sauces: { ru: "Соусы и масла", en: "Sauces & Oils", keywords: ["соус", "масло раст", "масло олив", "уксус", "соев", "горчиц", "кетчуп", "майонез", "песто"] },
  other: { ru: "Прочее", en: "Other", keywords: [] }
};
...
export default function CookifyDemo() {
  ...
  // Планировщик меню
  const [accountTab, setAccountTab] = useState("history"); // history | planner | shopping
  ...
  // Список покупок
  const [purchasedItems, setPurchasedItems] = useState({}); // { "ingredient_name": true/false }
  ...
  useEffect(() => {
    ...
    const savedPurchasedItems = localStorage.getItem("cookify_purchasedItems");
    ...
    if (savedPurchasedItems) setPurchasedItems(JSON.parse(savedPurchasedItems));
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem("cookify_purchasedItems", JSON.stringify(purchasedItems));
  }, [purchasedItems]);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("cookify_purchasedItems");
  };
  ...
  // Функция для извлечения чистого названия ингредиента (без количества)
  const extractIngredientName = (ingredient) => {
    // Удаляем числа, единицы измерения и распространенные предлоги
    return ingredient
      .replace(/\d+/g, '') // удаляем числа
      .replace(/г\b|мл\b|кг\b|л\b|шт\b|ст\.л\.|ч\.л\.|стакан|чашк|по вкусу/gi, '') // единицы измерения
      .trim();
  };

  // Получить список ингредиентов из плана меню
  const generateShoppingList = () => {
    const weekDays = getWeekDays(plannerWeekDate);
    const allIngredients = [];

    // Собираем все ингредиенты со всех дней недели
    ...
    // Группируем по категориям и объединяем одинаковые ингредиенты
    const grouped = {};
    
    allIngredients.forEach(({ original, name, category }) => {
      if (!grouped[category]) {
        grouped[category] = {};
      }
      ...
      if (!grouped[category][key]) {
        ...
        grouped[category][key].count += 1;
        if (grouped[category][key].examples.length < 2) {
          grouped[category][key].examples.push(original);
        }
      }
    });
  ...
  const togglePurchased = (ingredientKey) => {
  ...
  // Очистить список покупок (сбросить все отметки)
  const clearShoppingList = () => {
    setPurchasedItems({});
  };

  // Экспорт списка в текст
  const exportShoppingList = () => {
    let text = language === "ru" ? "Список покупок\n\n" : "Shopping List\n\n";
    text += `${getWeekRange(plannerWeekDate, language)}\n\n`;

    Object.entries(shoppingList).forEach(([categoryKey, items]) => {
      const categoryName = INGREDIENT_CATEGORIES[categoryKey][language];
      text += `${categoryName}:\n`;
      
      Object.entries(items).forEach(([key, data]) => {
        const checkbox = purchasedItems[key] ? "[x]" : "[ ]";
        const countText = data.count > 1 ? ` (×${data.count})` : "";
        text += `  ${checkbox} ${data.name}${countText}\n`;
      });
    ...
    a.href = url;
    a.download = `shopping-list-${plannerWeekDate}.txt`;
